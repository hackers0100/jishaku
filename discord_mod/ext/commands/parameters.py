...
from discord_mod.utils import MISSING, maybe_coroutine
...
class Parameter(inspect.Parameter):
    ...
    def converter(self) -> Any:
        ...
        if self.annotation is empty:
            return type(self.default) if self.default not in (empty, None) else str # type: ignore
    ...
    *,
    ...
    if isinstance(default, Parameter):
        if displayed_default is empty:
            displayed_default = default._displayed_default # type: ignore

        default = default._default # type: ignore

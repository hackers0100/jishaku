...
def entrypoint(
    ...
    intents_class = discord_mod.Intents.default()
    all_intents = [name for name, _ in discord_mod.Intents.all()]
    default_intents = [name for name, value in discord_mod.Intents.default() if value]
    ...
    for intent in intents:
        ...
        if name in all_intents:
            ...
            intents_class = discord_mod.Intents.all() if value else discord_mod.Intents.none()
        elif name == 'default':
    ...
    def prefix(bot: commands.Bot, _: discord_mod.Message) -> typing.List[str]:
        return [

...
class RootCommand(Feature):
    ...
    async def jsk(self, ctx: ContextA):
        ...
        if distributions:
            ...
            dist_version = f'unknown `{discord_mod.__version__}`'
        ...
        if isinstance(self.bot, discord_mod.AutoShardedClient):
    ...
    async def jsk_cancel(self, ctx: ContextA, *, index: typing.Union[int, str]):
        ...
        if index == -1:
            ...
            task = discord_mod.utils.get(self.tasks, index=index)
            if task:
        ...
        if task.ctx.command:
            await ctx.send(f"Cancelled task {task.index}: `{task.ctx.command.qualified_name}`,"
                           f" invoked {discord_mod.utils.format_dt(task.ctx.message.created_at, 'R')}")
        else:

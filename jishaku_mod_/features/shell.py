...
from jishaku_mod_.paginators import PaginatorInterface, WrappedPaginator
...
class ShellFeature(Feature):
    ...
    class ShellStandardInputModal(ui.Modal, title="Type input"):
        ...
        stdin_content: ui.TextInput[ui.Modal] = ui.TextInput(label="Text", style=discord_mod.TextStyle.short)
        ...
        async with ReplResponseReactor(ctx.message):
            with self.submit(ctx):
                with ShellReader(argument.content, escape_ansi=not Flags.use_ansi(ctx)) as reader:
                    ...
                    async def send_standard_input(interaction: discord_mod.Interaction):

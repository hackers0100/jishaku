...
import discord_mod
import discord_mod.opus
import discord_mod.voice_client
...
class VoiceFeature(Feature):
    ...
    async def voice_check(ctx: ContextA):
        ...
        if not discord_mod.voice_client.has_nacl:
            return await ctx.send("Voice cannot be used because PyNaCl is not loaded.")

        if not discord_mod.opus.is_loaded():
            if hasattr(discord_mod.opus, '_load_default'):
                if not discord_mod.opus._load_default():  # type: ignore  # pylint: disable=protected-access,no-member
    ...
    async def connected_check(ctx: ContextA):
        ...
        if not ctx.guild or not ctx.guild.voice_client or (
            ...
            if isinstance(ctx.guild.voice_client, discord_mod.VoiceClient)
            else False
        ):
    ...
    async def playing_check(ctx: ContextA):
        ...
        guild: discord_mod.Guild = ctx.guild  # type: ignore

        if (not guild.voice_client.is_playing() if isinstance(guild.voice_client, discord_mod.VoiceClient) else False):
    ...
    async def jsk_voice(self, ctx: ContextA):
        ...
        voice: discord_mod.VoiceProtocol = guild.voice_client  # type: ignore

        if isinstance(voice, discord_mod.VoiceClient):
    ...
    async def jsk_vc_join(
        ...
        ctx: ContextA,
        *,
        destination: typing.Union[discord_mod.VoiceChannel, discord_mod.Member] = None  # type: ignore
    ):
        """
        ...
        if isinstance(destination, discord_mod.Member):
            if destination.voice and destination.voice.channel:
                if isinstance(destination.voice.channel, discord_mod.StageChannel):
        ...
        voice: discord_mod.VoiceProtocol = ctx.guild.voice_client  # type: ignore
    ...
    async def jsk_vc_volume(self, ctx: ContextA, *, percentage: float):
        ...
        if isinstance(voice, discord_mod.VoiceClient):
            ...
            if not isinstance(source, discord_mod.PCMVolumeTransformer):
    ...
    async def jsk_vc_play(self, ctx: ContextA, *, uri: str):
        ...
        if isinstance(voice, discord_mod.VoiceClient):
            ...
            voice.play(discord_mod.PCMVolumeTransformer(discord_mod.FFmpegPCMAudio(uri)))

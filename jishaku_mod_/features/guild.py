...
class GuildFeature(Feature):
    ...
    def apply_overwrites(
        ...
        allow_p: discord_mod.Permissions = discord_mod.Permissions(allow)
        deny_p: discord_mod.Permissions = discord_mod.Permissions(deny)
    ...
    async def jsk_permtrace(
        self,
        ctx: ContextA,
        channel: typing.Union[discord_mod.TextChannel, discord_mod.VoiceChannel],
        *targets: typing.Union[discord_mod.Member, discord_mod.Role]
    ):
        """
        ...
        member_ids = {target.id: target for target in targets if isinstance(target, discord_mod.Member)}
        roles: typing.List[discord_mod.Role] = []

        for target in targets:
            if isinstance(target, discord_mod.Member):
        ...
        if member_ids and channel.guild.owner_id in member_ids:
            ...
            for key in dict(discord_mod.Permissions.all()).keys():
            ...
            if not is_administrator:
                ...
                def is_role(overwrite: discord_mod.abc._Overwrites) -> bool:  # type: ignore
                    return overwrite.is_role()

                def is_member(overwrite: discord_mod.abc._Overwrites) -> bool:  # type: ignore
        ...
        embed = discord_mod.Embed(color=0x00FF00, description=description)

...
from jishaku_mod_.models import copy_context_with
from jishaku_mod_.paginators import PaginatorInterface, WrappedPaginator, use_file_check
from jishaku_mod_.types import ContextA, ContextT

UserIDConverter = commands.IDConverter[typing.Union[discord_mod.Member, discord_mod.User]]
ChannelIDConverter = commands.IDConverter[
    typing.Union[discord_mod.abc.GuildChannel, discord_mod.abc.PrivateChannel, discord_mod.Thread]
]
...
class SlimUserConverter(UserIDConverter):  # pylint: disable=too-few-public-methods
    ...
    async def convert(self, ctx: ContextA, argument: str) -> typing.Union[discord_mod.Member, discord_mod.User]:
        """Converter method"""
        match = self._get_id_match(argument) or re.match(r"<@!?([0-9]{15,20})>$", argument)  # type: ignore

        if match is not None:
            user_id = int(match.group(1))
            result = ctx.bot.get_user(user_id) or discord_mod.utils.get(ctx.message.mentions, id=user_id)
            if result is None:
                try:
...
class InvocationFeature(Feature):
    ...
    async def jsk_override(self, ctx: ContextT, overrides: commands.Greedy[OVERRIDE_SIGNATURE], *, command_string: str):
        ...
        for override in overrides:
            if isinstance(override, discord_mod.User):
                # This is a user
                if ctx.guild:
                    ...
                    with contextlib.suppress(discord_mod.HTTPException):
    ...
    async def jsk_source(self, ctx: ContextA, *, command_name: str):
        ...
        if use_file_check(ctx, len(source_text)):  # File "full content" preview limit
            await ctx.send(file=discord_mod.File(filename=filename, fp=io.BytesIO(source_text.encode("utf-8"))))
        else:
